...
2.2.3 arm64 已通过英语入口真实会话的幂等设置、toggle、stop 和正常重启自动加载。实际文字/候选上屏待确认；跨应用记忆未专项测试。
历史上出现过大量重复输入源和卡顿，重新登录后恢复，根因未确认。若重现，停测试，不循环注册或全局清空偏好。
回退只处理测试 app；不要删除或擅自恢复当前词库。原厂启动曾自动修复 LevelDB，不能据启动成功推断数据完整。
